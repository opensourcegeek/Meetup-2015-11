...
Elm.Main.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var isRed = function (c) {    return _U.eq(c,"Red");};
   var mkBigNode = function (color) {
      return A2($Html.div,
      _U.list([$Html$Attributes.classList(_U.list([{ctor: "_Tuple2"
                                                   ,_0: "bigBlocks"
                                                   ,_1: true}
                                                  ,{ctor: "_Tuple2",_0: "hoursRed",_1: isRed(color)}]))]),
      _U.list([]));
   };
   var getBigColouredNodes = function (colors) {
      return A2($List.map,mkBigNode,colors);
   };
   var mkNode = function (color) {
      return A2($Html.div,
      _U.list([$Html$Attributes.classList(_U.list([{ctor: "_Tuple2"
                                                   ,_0: "blocks"
                                                   ,_1: true}
   ...
   var getColouredNodes = function (colors) {
      return A2($List.map,mkNode,colors);
   };
   var getColours = function (ts) {
      return A2($List.map,
      function (x) {
         return _U.eq(x,true) ? "Red" : "None";
      },
      ts);
   };
   ...
   var topRowMinutes = function (m) {
      ...
      11,
      A2($Basics._op["++"],
      A2($List.repeat,m / minutesFactor | 0,true),
      A2($List.repeat,11,false)));
   };
   var bottomRowMinutes = function (m) {
      return A2($List.take,
      4,
      A2($Basics._op["++"],
      A2($List.repeat,A2($Basics.rem,m,minutesFactor),true),
      A2($List.repeat,4,false)));
   };
   var hoursFactor = 5;
   var topRowHours = function (h) {
      return A2($List.take,
      4,
      A2($Basics._op["++"],
      A2($List.repeat,h / hoursFactor | 0,true),
      A2($List.repeat,4,false)));
   };
   var bottomRowHours = function (h) {
      return A2($List.take,
      4,
      A2($Basics._op["++"],
      A2($List.repeat,A2($Basics.rem,h,hoursFactor),true),
      A2($List.repeat,4,false)));
   };
   var currentTime = function (t) {
      ...
      _U.list([$Html.text(A2($Basics._op["++"],
              ...
              ,A2($Html.div,
              _U.list([]),
              getBigColouredNodes(getColours(bottomRowHours($Date.hour(date$)))))
              ,A2($Html.div,
              _U.list([]),
              getColouredNodes(getColours(topRowMinutes($Date.minute(date$)))))
              ,A2($Html.div,
              _U.list([]),
              getBigColouredNodes(getColours(bottomRowMinutes($Date.minute(date$)))))]));
   };
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,bottomRowMinutes: bottomRowMinutes
